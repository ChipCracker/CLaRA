...
The goal is a reproducible toolchain that checks LaTeX documents locally. The suite combines \emph{ChkTeX}, \emph{Vale}, \emph{LanguageTool}, and optional LLM-assisted hints.
% clara: ignore-next-line
Well, thats definately fine.

\section{Example}
A short formula: \(f(x) = x^2 + \int_0^1 t\,dt\).

\input{sections/unter}
...
This is an intential typo. % CLaRA-FIX: corrected 'intential' → 'intentional'
\end{document}
